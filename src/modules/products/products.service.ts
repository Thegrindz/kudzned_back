...
export class ProductsService {
  ...
  async findAll(query: ProductQueryDto): Promise<StandardResponse<any>> {
    try {
      ...
      return this.responseService.paginated(products, page, limit, total, 'Products retrieved successfully');
    } catch (error) {
      return this.responseService.internalServerError('Failed to retrieve products', { error: error.message });
    }
  }
  ...
  async findById(id: string): Promise<StandardResponse<Product>> {
    try {
      ...
      return this.responseService.success('Product retrieved successfully', product);
    } catch (error) {
      return this.responseService.internalServerError('Failed to retrieve product', { error: error.message });
    }
  }
  ...
  async findFeatured(limit = 10): Promise<StandardResponse<Product[]>> {
    try {
      ...
      return this.responseService.success('Featured products retrieved successfully', products);
    } catch (error) {
      return this.responseService.internalServerError('Failed to retrieve featured products', { error: error.message });
    }
  }
  ...
  async create(
    ...
    try {
      const { image, additional_images, digital_file, tags, ...productData } = createProductDto;
      ...
      if (tags) {
        if (typeof tags === 'string') {
          ...
          processedTags = tags.filter(tag => typeof tag === 'string' && tag.length > 0);
        }
      }
      ...
      if (files?.image && files.image[0]) {
        try {
          ...
          return this.responseService.internalServerError('Failed to upload main image', { 
      ...
      if (files?.additional_images && files.additional_images.length > 0) {
        try {
          for (const file of files.additional_images.slice(0, 5)) { // Max 5 additional images
            const uploadResult = await this.cloudinaryService.uploadFile(
              file, 
              'products/images'
            );
          ...
          return this.responseService.internalServerError('Failed to upload additional images', { 
            error: uploadError.message 
          });
        }
      }
      ...
      if (files?.digital_file && files.digital_file[0]) {
        try {
          ...
          return this.responseService.internalServerError('Failed to upload digital file', { 
            error: uploadError.message 
          });
        }
      }
      ...
      return this.responseService.created('Product created successfully', completeProduct);
    } catch (error) {
      return this.responseService.internalServerError('Failed to create product', { error: error.message });
    }
  }
  ...
  async update(
    ...
    try {
      ...
      if (userRole !== UserRole.ADMIN && product.vendor_id !== userId) {
        return this.responseService.forbidden('You can only update your own products');
      }
      ...
      if (files?.additional_images && files.additional_images.length > 0) {
        try {
          ...
          updatedImages = [updatedImages[0], ...newImageUrls].filter(Boolean).slice(0, 6); // Max 6 total images
        } catch (uploadError) {
      ...
      if (updatedProductResponse.success) {
        return this.responseService.success('Product updated successfully', updatedProductResponse.data);
      }
      
      return updatedProductResponse;
    } catch (error) {
      return this.responseService.internalServerError('Failed to update product', { error: error.message });
    }
  }

  async delete(id: string, userId: string, userRole: UserRole): Promise<StandardResponse<any>> {
    try {
      const productResponse = await this.findById(id);
      if (!productResponse.success) {
        return productResponse;
      ...
      // Check permissions
      if (userRole !== UserRole.ADMIN && product.vendor_id !== userId) {
        return this.responseService.forbidden('You can only delete your own products');
      }

      await this.productRepository.delete(id);
      return this.responseService.success('Product deleted successfully', { success: true });
    } catch (error) {
      return this.responseService.internalServerError('Failed to delete product', { error: error.message });
    }
  }

  async getVendorProducts(vendorId: string, query: ProductQueryDto): Promise<StandardResponse<any>> {
    try {
      const {
        page = 1,
        limit = 20,
      ...
      return this.responseService.paginated(products, page, limit, total, 'Vendor products retrieved successfully');
    } catch (error) {
      return this.responseService.internalServerError('Failed to retrieve vendor products', { error: error.message });
    }
  }

  async addDigitalFile(productId: string, fileData: {
    filename: string;
    ...
    try {
      ...
      return this.responseService.created('Digital file added successfully', savedFile);
    } catch (error) {
      return this.responseService.internalServerError('Failed to add digital file', { error: error.message });
    }
  }
  ...
  async removeDigitalFile(fileId: string): Promise<StandardResponse<any>> {
    try {
      ...
      return this.responseService.success('Digital file removed successfully', { success: true });
    } catch (error) {
      return this.responseService.internalServerError('Failed to remove digital file', { error: error.message });
    }
  }

  async incrementSales(productId: string): Promise<StandardResponse<any>> {
    try {
      await this.productRepository.increment({ id: productId }, 'total_sales', 1);
      return this.responseService.success('Product sales incremented successfully', { success: true });
    } catch (error) {
      return this.responseService.internalServerError('Failed to increment product sales', { error: error.message });
    }
  }
}
