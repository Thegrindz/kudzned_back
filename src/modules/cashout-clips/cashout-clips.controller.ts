...
export class CashoutClipsController {
  ...
  @ApiOperation({ 
    ...
    description: 'Upload a video showing successful cashout from a product. Requires authentication.' 
  })
 ...
 @ApiBearerAuth('JWT-auth')
  ...
  async create(
    ...
    return this.cashoutClipsService.create(req.user.id, createCashoutClipDto, files);
  }

  @Get()
  @ApiOperation({ 
    summary: 'Get all cashout clips',
    description: 'Retrieve cashout clips with optional filtering and pagination' 
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiQuery({ name: 'limit', required: false, description: 'Number of clips to return (default: 10)' })
  @ApiResponse({
    status: 200,
  ...
  async update(
    ...
    return this.cashoutClipsService.update(id, req.user.id, updateCashoutClipDto, files);
  }
...
@ApiBearerAuth('JWT-auth')
 ...
 @ApiBearerAuth('JWT-auth')
  ...
  @ApiOperation({ 
    ...
    description: 'Approve, reject, or flag a cashout clip. Admin access required.' 
  })
 @ApiBearerAuth('JWT-auth')
  @ApiParam({ name: 'id', description: 'Cashout clip UUID' })
  @ApiQuery({ name: 'status', enum: CashoutClipStatus, description: 'New status for the clip' })
  @ApiQuery({ name: 'reason', required: false, description: 'Rejection reason (if rejecting)' })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({ 
    ...
    description: 'Toggle the featured status of a cashout clip. Admin access required.' 
  })
  @ApiBearerAuth('JWT-auth')
  @ApiParam({ name: 'id', description: 'Cashout clip UUID' })
  @ApiResponse({
