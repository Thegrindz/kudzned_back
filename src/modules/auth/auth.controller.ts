...
import { AuthResponseDto, TwoFASetupResponseDto, RefreshTokenResponseDto, UserResponseDto } from './dto/auth-response.dto';
...
export class AuthController {
  ...
  @ApiOperation({ 
    ...
    description: 'Create a new user account with specified role (customer, vendor, or admin). Defaults to customer if no role specified.'
  })
  @ApiBody({ 
    type: RegisterDto,
    examples: {
  ...
  async register(@Body() registerDto: RegisterDto): Promise<StandardResponse<AuthResponseDto>> {
  ...
  async login(@Body() loginDto: LoginDto): Promise<StandardResponse<AuthResponseDto>> {
  ...
  async verify2FA(@Req() req, @Body() twoFADto: TwoFADto): Promise<StandardResponse<{ verified: boolean }>> {
  ...
  @UseGuards(JwtAuthGuard)
  async enable2FA(@Req() req): Promise<StandardResponse<TwoFASetupResponseDto>> {
  ...
  @UseGuards(JwtAuthGuard)
  async disable2FA(@Req() req): Promise<StandardResponse<{ success: boolean }>> {
  ...
  @UseGuards(JwtAuthGuard)
  async refreshToken(@Req() req): Promise<StandardResponse<RefreshTokenResponseDto>> {
  ...
  async getProfile(@Req() req): Promise<StandardResponse<UserResponseDto>> {
    ...
    const responseService = new (await import('../../common/services/response.service')).ResponseService();
    console.log(req.user)
    return responseService.success('User information retrieved successfully', req.user);
  }
}