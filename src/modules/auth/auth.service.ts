...
export class AuthService {
  ...
  async login(loginDto: LoginDto): Promise<StandardResponse<any>> {
    try {
      ...
      return this.responseService.internalServerError("Login failed", {
        error: error.message,
      });
    }
  }
  ...
  async verify2FA(
    ...
    try {
      ...
      return this.responseService.success("2FA verification successful", {
        verified: true,
      });
    } catch (error) {
      return this.responseService.internalServerError(
  ...
  async enable2FA(userId: string): Promise<StandardResponse<any>> {
    try {
      ...
      return this.responseService.internalServerError("Failed to enable 2FA", {
        error: error.message,
      });
    }
  }
  ...
  async disable2FA(userId: string): Promise<StandardResponse<any>> {
    try {
      ...
      await this.notificationsService.createNotification({
        ...
        title: "2FA Disabled",
        message:
          "Two-Factor Authentication is now disabled. Your account is less secure.",
        skipEmail: true,
      });
      ...
      return this.responseService.internalServerError("Failed to disable 2FA", {
        error: error.message,
      });
    }
  }
  ...
  async refreshToken(userId: string): Promise<StandardResponse<any>> {
    try {
      ...
      return this.responseService.success("Token refreshed successfully", {
        access_token,
      });
    } catch (error) {
      return this.responseService.internalServerError(
